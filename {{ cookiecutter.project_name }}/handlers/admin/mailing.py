...
async def run_mailing(call: CallbackQuery, state: FSMContext):
    mailing_message = (
        ...
        .hide_keyboard_after()
        .edit(message_id=call.message.message_id)
    )

    await state.update_data(
        mailing_message=[mailing_message.message.message_id]
            ...
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="✅️ Отправить", callback_data=f"send-mail"
                    ),
...
async def start_mailing(call: CallbackQuery, state: FSMContext):
    try:
        ...
        await bot.send_message(
            chat_id=call.message.chat.id, text="<b>Рассылка начата</b>"
        )
...
@router.callback_query(
    lambda call: call.data == "cancel-mail", flags={"is_admin": True}
)
